---
name: pdf
description: 텍스트 및 테이블 추출, 새 PDF 생성, 문서 병합/분할 및 양식 처리를 위한 포괄적인 PDF 조작 툴킷. Claude가 PDF 양식을 작성하거나 대규모로 PDF 문서를 프로그래밍 방식으로 처리, 생성 또는 분석해야 할 때.
...
이 가이드는 Python 라이브러리 및 명령줄 도구를 사용한 필수 PDF 처리 작업을 다룹니다. 고급 기능, JavaScript 라이브러리 및 자세한 예제는 reference.md를 참조하세요. PDF 양식을 작성해야 하는 경우 forms.md를 읽고 지침을 따르세요.
...
# 이것은 모든 이미지를 output_prefix-000.jpg, output_prefix-001.jpg 등으로 추출합니다.
```
